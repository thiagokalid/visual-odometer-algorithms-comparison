...
for description in trajectory_description:
    shift_x, shift_y = description["shift_x"] / downscale_factor, description["shift_y"] / downscale_factor
    N = description["N"]
    ...
    for i in range(1, N):
        ...
        img_int = base_img[int(int_z - img_size[0]//2) : int(int_z + img_size[0]//2), int(int_x - img_size[1]//2) : int(int_x + img_size[1]//2)]
        ...
        shifted_img = add_gaussian_noise(shifted_img, sigma=10)
        ...
        data['delta_x'].append(rand_delta_x)
        data['delta_y'].append(rand_delta_y)
